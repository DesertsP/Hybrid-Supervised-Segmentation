...
class DDPEngine(object):
    ...
    def train_epoch(self, epoch):
        ...
        for step, (data_one, data_two) in enumerate(zip(self.train_loader, self.train_loader_unsup)):
            ...
            # ---
            if global_step >= self.config.train.indicator_update_from_iter and global_step % self.config.train.indicator_update_freq == 0:
    ...
    def train_indicator(self):
        ...
        if self.unbiased_iter is None:
            self.unbiased_iter = iter(self.unbiased_loader)
        try:
            unbiased_data = next(self.unbiased_iter)
        except:
            self.unbiased_iter = iter(self.unbiased_loader)
            unbiased_data = next(self.unbiased_iter)
        self.model(unbiased_data['image'].cuda(), unbiased_data['target'].cuda(), mode=2)
        unbiased_elapse = time.time() - start_time
        for step, data in enumerate(self.indicator_loader):
            self.model(data['image'].cuda(), target=None, index=data['index'].cuda(), mode=3)
