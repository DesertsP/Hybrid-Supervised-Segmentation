...
class DDPEngine(object):
    ...
    def train_epoch(self, epoch):
        ...
        self.model.train()
        batch_end = time.time()
        for step, ((image, target), (image_unsup, _)) in enumerate(zip(self.train_loader, self.train_loader_unsup)):
            batch_start = time.time()
            data_times.update(batch_start - batch_end)
            global_step = len(self.train_loader) * epoch + step
            output = self.model(image.cuda(), target.cuda(), image_unsup.cuda())
