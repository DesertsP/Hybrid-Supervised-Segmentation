...
class DeepLab(nn.Module):
    def __init__(self, num_classes, repr_channels, backbone_cfg: dict, decoder_cfg: dict, norm_cfg: dict):
        ...
        self.repr_projector = nn.Sequential(
            ...
            ConvModule(256, 256, 3, padding=1, bias=True, norm_layer=norm_layer, relu_inplace=True),
            nn.Dropout2d(0.1),
            nn.Conv2d(256, repr_channels, kernel_size=1, stride=1, padding=0, bias=True),
        )
        ...
        self._init_weight(self.repr_projector)

    def forward(self, x):
        x = self.backbone(x)
        x = self.decoder(x)
        repr_x = self.repr_projector(x)
        x = self.projector(x)
        pred = self.classifier(x)
        return {'pred': pred, 'repr': repr_x}
