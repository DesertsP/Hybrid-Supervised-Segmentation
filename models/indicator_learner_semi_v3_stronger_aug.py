...
class IndicatorLearner(nn.Module):
    ...
    def compute_influence(self, feat, target, feat_u, target_u, sync=True):
        ...
        if self.hessian_type == 'identity':
            ...
            hess_inv = torch.linalg.pinv(self.hessian_buffer)
    ...
    def apply_augmentation(self, image, target, index, mix_mask=None):
        ...
        indicator_p = label_onehot(target, self.num_classes, ignore_index=self.ignore_index)
    ...
    def forward_train(self, image, target, image_u, target_u, index_u):
        ...
        # stronger augmentation on fully labeled images
        image, target, _ = self.apply_augmentation(image, target,
                                                   index=torch.zeros(image.size(0), dtype=torch.long, device=image.device),
                                                   mix_mask=mix_mask)
        image = self.augment(image)
        image_u = self.strong_augment(image_u)
        image_all = torch.cat([image, image_u], dim=0)
        ...
        pseudo_loss = self.criterion_pseudo(resize_as(pred_u, image_u), target_u)
        pseudo_loss *= indicator_u
        pseudo_loss = pseudo_loss.mean()
        results = dict(seg_loss=sup_loss, reg_loss=pseudo_loss)

        if 'aux' in outputs:
            aux_all = outputs['aux']
            aux, aux_u = torch.split(aux_all, [num_labeled, num_unlabeled], dim=0)
            aux_loss = self.criterion(resize_as(aux, image), target).mean()
            results.update(dict(aux_loss=aux_loss))
        # moving average
    ...
    def parameter_groups(self):
        ...
        if self.online_net.aux_output:
            newly_added.append(self.online_net.aux_classifier)
