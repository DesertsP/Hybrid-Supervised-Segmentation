...
from models.losses.reco_ori import compute_reco_loss
...
class MeanTeacherLearner(nn.Module):
    ...
    def compute_indicator_grad(self, image, target=None, index=None, sync=True):
        ...
        if target is None:
            target = self.forward_pseudo_label(image, index, use_augment=False)[1]
    ...
    def forward_pseudo_label(self, image, index, use_augment=True):
        ...
        prob_u, label_u = torch.max(pred_u_teacher, dim=1)
        ...
        if use_augment and self.pseudo_augment and np.random.uniform(0, 1) < self.pseudo_augment_prob:
            ...
            prob_u = prob_u * mix_mask + prob_u.roll(-1, dims=0) * (1 - mix_mask)
        ...
        return image, label_u, prob_u, indicator_p

    def forward_train(self, image, target, image_u, index_u):
        image_u, target_u, prob_u, indicator_u = self.forward_pseudo_label(image_u.clone(), index_u)
        ...
        # contrastive loss
        proj_all = outputs["repr"]
        with torch.no_grad():
            valid_mask_u = prob_u.ge(0.7).float()
            valid_mask_all = torch.cat([(target.unsqueeze(1) >= 0).float(), valid_mask_u.unsqueeze(1)], dim=0)
            valid_mask_all = F.interpolate(valid_mask_all, size=proj_all.shape[2:], mode='nearest')
            target_all = torch.cat([target, target_u], dim=0)
            target_all_oh = F.interpolate(label_onehot(target_all, self.num_classes), size=proj_all.shape[2:], mode='nearest')
            prob_all = torch.softmax(pred_all, dim=1)
        cl_loss = compute_reco_loss(proj_all, target_all_oh, valid_mask_all, prob_all)

        # moving average
        update_model_moving_average(self.ema_decay, self.target_net, self.online_net)
        return dict(seg_loss=sup_loss, reg_loss=pseudo_loss, cl_loss=cl_loss)
    ...
    def parameter_groups(self):
        ...
        newly_added = [self.online_net.decoder,
                       ...
                       self.online_net.repr_projector
                       ]
