...
class CrossPseudoLearner(nn.Module):
    ...
    def forward_train(self, input, target=None, mask=None, input_u=None, mask_u=None):
        seg_loss_one = self.forward_loss(input, target, mask, branch=1)
        seg_loss_two = self.forward_loss(input, target, mask, branch=2)
        pseudo_one = self.forward_pseudo(input_u, branch=1)
        pseudo_two = self.forward_pseudo(input_u, branch=2)
        pseudo_loss_one = self.forward_loss(input_u, pseudo_two, mask_u, branch=1)
        pseudo_loss_two = self.forward_loss(input_u, pseudo_one, mask_u, branch=2)
        return dict(seg_loss=seg_loss_one+seg_loss_two, reg_loss=pseudo_loss_one+pseudo_loss_two)
