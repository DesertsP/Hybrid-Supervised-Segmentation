...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(mask_path):
            mask = np.array(Image.open(get_segmentation_path(name_str, self.voc12_root)))
        else:
...
class VOC12ClassificationDatasetTest(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        out = {"name": name_str, "img": ms_img_list, "size": (img.shape[0], img.shape[1]),
               ...
               "mask": torch.zeros(img.shape[0], img.shape[1], dtype=torch.int)}
        return out
