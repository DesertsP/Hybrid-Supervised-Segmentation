...
from utils import config_parser, build_module, init_log, AverageMeter, make_infinite
...
# torch.autograd.set_detect_anomaly(True)   # NOT supported by functorch
...
class DDPEngine(object):
    def __init__(self, config, logger, local_rank=-1):
        ...
        self.unbiased_data_iter = make_infinite(self.unbiased_loader)
        self.indicator_data_iter = make_infinite(self.indicator_loader)
        # ---
    ...
    def validate(self, epoch):
        ...
        ignore_label = self.train_set.ignore_label
    ...
    def train_indicator(self):
        ...
        # prior term
        unbiased_data = next(self.unbiased_data_iter)
        image = unbiased_data['image'].cuda()
        target = unbiased_data['target'].cuda()
        # later term
        data = next(self.indicator_data_iter)
        image_u = data['image'].cuda()
        index_u = data['index'].cuda()
        target_u = data.get('target', None)
        if target_u is not None:
            target_u = target_u.cuda()
        #
        data_times = time.time() - start_time
        indicators = self.model(image, target, image_u, target_u, index_u, sync=True, mode=2)
        ...
        if self.local_rank <= 0:
            self.logger.info(
                "Indicator\t"
                "Data {data_times:.2f}\t\t"
                "Time {total_elapse:.2f}\t\t"
                "Mean {mean_val:.2f}\t"
                "Max {max_val:.2f}\t"
                "Min {min_val:.2f}\t".format(
